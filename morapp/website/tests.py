...
class AuthAPITests(APITestCase):
    ...
    def test_register(self):
        ...
        print(f"test_register: mail.outbox length={len(mail.outbox)}")
    ...
    def test_register_rate_limit(self):
        ...
        for i in range(10):
            ...
            print(f"test_register_rate_limit: request {i+1}, mail.outbox length={len(mail.outbox)}")
            self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        data = {
            'username': 'testuser10',
        ...
        response = self.client.post(self.register_url, data, format='json')
        print(f"test_register_rate_limit: 11th request, status={response.status_code}, mail.outbox length={len(mail.outbox)}")
    ...
    def test_verify_email(self):
        cache.clear()
        response = self.client.post(self.register_url, self.user_data, format='json')
        print(f"test_verify_email: after register, mail.outbox length={len(mail.outbox)}")
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        ...
        print(f"test_verify_email: after verify, mail.outbox length={len(mail.outbox)}")
    ...
    def test_login(self):
        response = self.client.post(self.register_url, self.user_data, format='json')
        print(f"test_login: after register, mail.outbox length={len(mail.outbox)}")
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(len(mail.outbox), 1)
        email = mail.outbox[0].body
        token = re.search(r'token=([a-f0-9-]+)', email).group(1)
        self.client.post(self.verify_email_url, {'token': token, 'email': 'test@example.com'}, format='json')
        print(f"test_login: after verify, mail.outbox length={len(mail.outbox)}")
        data = {'username': 'testuser', 'password': 'Test1234'}
        response = self.client.post(self.login_url, data, format='json')
        print(f"test_login: after login, mail.outbox length={len(mail.outbox)}")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertIn('access', response.data)
        self.assertIn('refresh', response.data)
    ...
    def test_profile(self):
        response = self.client.post(self.register_url, self.user_data, format='json')
        print(f"test_profile: after register, mail.outbox length={len(mail.outbox)}")
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(len(mail.outbox), 1)
        email = mail.outbox[0].body
        token = re.search(r'token=([a-f0-9-]+)', email).group(1)
        verify_response = self.client.post(self.verify_email_url, {'token': token, 'email': 'test@example.com'}, format='json')
        print(f"test_profile: after verify, mail.outbox length={len(mail.outbox)}")
        access_token = verify_response.data['access']
        self.client.credentials(HTTP_AUTHORIZATION=f'Bearer {access_token}')
        response = self.client.get(self.profile_url, format='json')
        print("Profile GET response:", response.status_code, response.data)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        print("Profile PUT response:", response.status_code, response.data)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['phone'], '+1234567890')

    def test_logout(self):
        response = self.client.post(self.register_url, self.user_data, format='json')
        print(f"test_logout: after register, mail.outbox length={len(mail.outbox)}")
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(len(mail.outbox), 1)
        email = mail.outbox[0].body
        token = re.search(r'token=([a-f0-9-]+)', email).group(1)
        verify_response = self.client.post(self.verify_email_url, {'token': token, 'email': 'test@example.com'}, format='json')
        print(f"test_logout: after verify, mail.outbox length={len(mail.outbox)}")
        ...
        print(f"test_logout: after logout, mail.outbox length={len(mail.outbox)}")
        self.assertEqual(response.status_code, status.HTTP_205_RESET_CONTENT)
        self.assertEqual(response.data['message'], 'Successfully logged out')

    def test_email_login(self):
        response = self.client.post(self.register_url, self.user_data, format='json')
        print(f"test_email_login: after register, mail.outbox length={len(mail.outbox)}")
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(len(mail.outbox), 1)
        email = mail.outbox[0].body
        token = re.search(r'token=([a-f0-9-]+)', email).group(1)
        self.client.post(self.verify_email_url, {'token': token, 'email': 'test@example.com'}, format='json')
        print(f"test_email_login: after verify, mail.outbox length={len(mail.outbox)}")
        response = self.client.post(self.email_login_url, {'email': 'test@example.com'}, format='json')
        print(f"test_email_login: after email login, mail.outbox length={len(mail.outbox)}")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_email_login_verify(self):
        response = self.client.post(self.register_url, self.user_data, format='json')
        print(f"test_email_login_verify: after register, mail.outbox length={len(mail.outbox)}")
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(len(mail.outbox), 1)
        email = mail.outbox[0].body
        token = re.search(r'token=([a-f0-9-]+)', email).group(1)
        self.client.post(self.verify_email_url, {'token': token, 'email': 'test@example.com'}, format='json')
        print(f"test_email_login_verify: after verify, mail.outbox length={len(mail.outbox)}")
        self.client.post(self.email_login_url, {'email': 'test@example.com'}, format='json')
        print(f"test_email_login_verify: after email login, mail.outbox length={len(mail.outbox)}")
        ...
        print(f"test_email_login_verify: after verify login, mail.outbox length={len(mail.outbox)}")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertIn('access', response.data)
        self.assertIn('refresh', response.data)
    ...
    def test_forgot_password(self):
        response = self.client.post(self.register_url, self.user_data, format='json')
        print(f"test_forgot_password: after register, mail.outbox length={len(mail.outbox)}")
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(len(mail.outbox), 1)
        email = mail.outbox[0].body
        token = re.search(r'token=([a-f0-9-]+)', email).group(1)
        self.client.post(self.verify_email_url, {'token': token, 'email': 'test@example.com'}, format='json')
        print(f"test_forgot_password: after verify, mail.outbox length={len(mail.outbox)}")
        response = self.client.post(self.forgot_password_url, {'email': 'test@example.com'}, format='json')
        print(f"test_forgot_password: after forgot password, mail.outbox length={len(mail.outbox)}")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_reset_password(self):
        cache.clear()
        response = self.client.post(self.register_url, self.user_data, format='json')
        print(f"test_reset_password: after register, mail.outbox length={len(mail.outbox)}")
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(len(mail.outbox), 1, f"Expected 1 email, got {len(mail.outbox)}")
        email = mail.outbox[0].body
        token = re.search(r'token=([a-f0-9-]+)', email).group(1)
        self.client.post(self.verify_email_url, {'token': token, 'email': 'test@example.com'}, format='json')
        print(f"test_reset_password: after verify, mail.outbox length={len(mail.outbox)}")
        self.client.post(self.forgot_password_url, {'email': 'test@example.com'}, format='json')
        print(f"test_reset_password: after forgot password, mail.outbox length={len(mail.outbox)}")
        self.assertEqual(len(mail.outbox), 2)
        ...
        print(f"test_reset_password: after reset password, mail.outbox length={len(mail.outbox)}")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        print(f"test_reset_password: after login, mail.outbox length={len(mail.outbox)}")
        self.assertEqual(response.status_code, status.HTTP_200_OK)